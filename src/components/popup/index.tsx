...
import { ResponseData } from '@/app/api/userData/route';
type Props = {
    userInfo: ResponseData
}

export default function Popup({ userInfo }: Props) {
    return (
        <Popover.Root>
            <Popover.Trigger className={styles.IconButton} aria-label='More info'>
            ...
            <Popover.Portal>
                <Popover.Positioner sideOffset={8}>
                    <Popover.Popup className={styles.Popup}>
                        ...
                        <Popover.Title className={styles.Title}>Pełne informacje</Popover.Title>
                        <Popover.Description className={styles.Description}>
                            Imię: {userInfo.imie ?? "not provided"} <br />
                            Nazwisko: {userInfo.nazwisko ?? "not provided"} <br />
                            Login: {userInfo.login ?? "not provided"} <br />
                            Email: {userInfo.email ?? "not provided"} <br />
                            Opis: {userInfo.opis ?? "not provided"} <br />
                            Nazwa: {userInfo.nazwa ?? "not provided"}
                        </Popover.Description>
                    </Popover.Popup>
                </Popover.Positioner>
            </Popover.Portal>
